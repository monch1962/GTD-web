...
export class BulkOperationsManager {
    ...
    updateBulkSelectedCount (): void {
        ...
        if (bulkSelectedCount) {
            bulkSelectedCount.textContent = this.state.selectedTaskIds.size.toString()
        }

        if (bulkCompleteBtn) {
            ;(bulkCompleteBtn as HTMLButtonElement).disabled = this.state.selectedTaskIds.size === 0
    ...
    bulkSelectAllVisible (): void {
        ...
        visibleTasks.forEach((taskElement) => {
            ...
            const checkbox = taskElement.querySelector('.bulk-select-checkbox') as HTMLInputElement
            if (checkbox && taskId) {
                this.state.selectedTaskIds.add(taskId)
    ...
    async bulkSetEnergy (): Promise<void> {
        ...
        for (const taskId of this.state.selectedTaskIds) {
            const task = this.state.tasks.find((t) => t.id === taskId)
            if (task) {
                task.energy = (energy as EnergyLevel) || ''
                task.updatedAt = new Date().toISOString()
            }
        }
