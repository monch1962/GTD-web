...
export class BulkOperationsManager {
    ...
    async bulkCompleteTasks() {
        ...
        this.app.showToast?.(`${this.state.selectedTaskIds.size} task(s) completed`);
    }
    ...
    async bulkAddContext() {
        ...
        this.exitBulkSelectionMode();
        this.app.renderView?.();
        this.app.showToast?.(`Added ${formattedContext} to ${this.state.selectedTaskIds.size} task(s)`);
    }
    ...
    async bulkDeleteTasks() {
        ...
        if (!confirm(`Are you sure you want to delete ${this.state.selectedTaskIds.size} task(s)?`)) {
            return;
        }

        this.app.saveState?.('Bulk delete tasks');
        this.state.tasks = this.state.tasks.filter(task => !this.state.selectedTaskIds.has(task.id));
        await this.app.saveTasks?.();
        this.exitBulkSelectionMode();
        this.app.renderView?.();
        this.app.updateCounts?.();
        this.app.showToast?.(`${this.state.selectedTaskIds.size} task(s) deleted`);
    }
}
