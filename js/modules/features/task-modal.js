...
export class TaskModalManager {
    ...
    openTaskModal(task = null, defaultProjectId = null, defaultData = {}) {
        ...
        if (task) {
            ...
            document.getElementById('task-status').value = this.state.currentView === 'all' ? 'inbox' : this.state.currentView;
    ...
    getRecurrenceLabel(recurrence) {
        ...
        if (typeof recurrence === 'object' && recurrence.type) {
            ...
            const details = [];
            if (recurrence.type === 'weekly' && recurrence.daysOfWeek) {
                const dayNames = recurrence.daysOfWeek.map(day => WeekdayNames[day] || day);
    ...
    renderWaitingForTasksList(currentTask) {
        ...
        let availableTasks = this.state.tasks.filter(t =>
    ...
    async saveTaskFromForm() {
        ...
        tags = tags.map(context => this.app.normalizeContextName?.(context) || (context.startsWith('@') ? context : `@${context}`));
