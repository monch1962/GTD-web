...
export class WeeklyReviewManager {
    ...
    renderWeeklyReview() {
        ...
        // Gather review data
        const completedThisWeek = this.state.tasks.filter(t =>
            t.completed && t.completedAt && new Date(t.completedAt) >= weekAgo
        );

        const incompleteTasks = this.state.tasks.filter(t => !t.completed && t.status !== 'someday' && t.status !== 'reference');
        ...
        const staleProjects = this.state.projects.filter(p => {
            ...
            const projectTasks = this.state.tasks.filter(t => t.projectId === p.id && !t.completed);
    ...
    async cleanupOldCompletedTasks() {
        ...
        const oldCompletedTasks = this.state.tasks.filter(t =>
