...
export class DailyReviewManager {
    ...
    renderDailyReview() {
        ...
        // Get tasks due today
        const dueToday = this.state.tasks.filter(t => !t.completed && t.dueDate && new Date(t.dueDate) <= today);

        // Get tasks due tomorrow
        const dueTomorrow = this.state.tasks.filter(t => !t.completed && t.dueDate && new Date(t.dueDate) >= tomorrow && new Date(t.dueDate) < new Date(tomorrow.getTime() + 24 * 60 * 60 * 1000));

        // Get overdue tasks
        const overdue = this.state.tasks.filter(t => !t.completed && t.dueDate && new Date(t.dueDate) < today);

        // Get high priority tasks
        const highPriority = this.state.tasks.filter(t => !t.completed && t.priority && t.priority >= 80);

        // Get tasks due this week
        const weekEnd = new Date(today);
        weekEnd.setDate(weekEnd.getDate() + 7);
        const dueThisWeek = this.state.tasks.filter(t => !t.completed && t.dueDate && new Date(t.dueDate) >= today && new Date(t.dueDate) < weekEnd);
     ...
     * @param {string} type - Type of task list (today, overdue, week, priority)
     */
     ...
     * @returns {string} Greeting (Morning, Afternoon, or Evening)
     */
    getGreeting() {
     ...
     * @returns {string} Personalized greeting with task count
     */
    getGreetingMessage() {
        const greeting = this.getGreeting();
        const totalTasks = this.state.tasks.filter(t => !t.completed).length;
        const completedToday = this.state.tasks.filter(t => t.completed && t.completedAt && new Date(t.completedAt) >= new Date(new Date().setHours(0, 0, 0, 0))).length;
