...
export class DashboardManager {
            ...
            <div>
                ...
                <div style="display: grid; gap: var(--spacing-sm);">
                    ${stalledProjects.map(project => {
                        const projectTasks = this.state.tasks.filter(t => t.projectId === project.id && !t.completed);
                        return `
    ...
    getVelocityTrend() {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            last7Days.push(this.state.tasks.filter(t => {
                if (!t.completedAt) return false;
                const completedDate = new Date(t.completedAt);
                return completedDate >= dayStart && completedDate < dayEnd;
            }).length);
        ...
        for (let i = 7; i < 14; i++) {
            ...
            prev7Days.push(this.state.tasks.filter(t => {
                if (!t.completedAt) return false;
                const completedDate = new Date(t.completedAt);
                return completedDate >= dayStart && completedDate < dayEnd;
            }).length);
