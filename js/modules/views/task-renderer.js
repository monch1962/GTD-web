...
export class TaskRenderer {
    ...
    _getFilteredTasks(additionalFilter = null) {
        ...
        if (this.state.currentProjectId) {
            filteredTasks = filteredTasks.filter(
                (task) => task.projectId === this.state.currentProjectId
            )
        } else {
            // Filter by view (only when not viewing a specific project)
            if (this.state.currentView !== 'all') {
                filteredTasks = filteredTasks.filter(
                    (task) => task.status === this.state.currentView
                )
            }
            ...
            if (this.state.currentView === 'next') {
                filteredTasks = filteredTasks.filter((task) =>
                    task.areDependenciesMet(this.state.tasks)
                )
            }
        }
        ...
        if (this.state.filters.context) {
            filteredTasks = filteredTasks.filter(
                (task) => task.contexts && task.contexts.includes(this.state.filters.context)
            )
        }
        ...
        if (this.state.selectedContextFilters && this.state.selectedContextFilters.size > 0) {
            filteredTasks = filteredTasks.filter((task) => {
                ...
                return task.contexts.some((context) =>
                    this.state.selectedContextFilters.has(context)
                )
            })
        }
        ...
        if (this.state.filters.energy) {
            filteredTasks = filteredTasks.filter(
                (task) => task.energy === this.state.filters.energy
            )
        }
    ...
    _attachTaskListeners(element, task) {
        ...
        // Timer button
        const timerBtn = element.querySelector(
            '.task-action-btn.timer, .task-action-btn.timer-active'
        )
        if (timerBtn) {
    ...
    _attachDragListeners(element, task) {
        ...
        element.addEventListener('dragover', (e) => {
            ...
            if (draggingItem && draggingItem !== element) {
                // Always reorder tasks, regardless of view
        ...
        element.addEventListener('drop', async (e) => {
            ...
            // Always update task positions on drop
            await this.app.updateTaskPositions?.()
        })
    }
