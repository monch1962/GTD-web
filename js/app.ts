...
import { escapeHtml, announce } from './dom-utils'
import { Task, Project, Template } from './models'
import { ArchiveManager } from './modules/features/archive'
import { BulkOperationsManager } from './modules/features/bulk-operations'
import { CalendarManager } from './modules/features/calendar'
import { ContextFilterManager } from './modules/features/context-filter'
import { DailyReviewManager } from './modules/features/daily-review'
import { DashboardManager } from './modules/features/dashboard'
import { DataExportImportManager } from './modules/features/data-export-import'
import { DependenciesManager } from './modules/features/dependencies'
import { FocusPomodoroManager } from './modules/features/focus-pomodoro'
import { GlobalQuickCaptureManager } from './modules/features/global-quick-capture'
import { NavigationManager } from './modules/features/navigation'
import { NewProjectButtonManager } from './modules/features/new-project-button'
import { PriorityScoringManager } from './modules/features/priority-scoring'
import { ProductivityHeatmapManager } from './modules/features/productivity-heatmap'
import { ProjectModalManager } from './modules/features/project-modal'
import { ProjectOperations } from './modules/features/project-operations'
import { QuickCaptureWidgetManager } from './modules/features/quick-capture-widget'
import { SearchManager } from './modules/features/search'
import { SmartDateSuggestionsManager } from './modules/features/smart-date-suggestions'
import { SmartSuggestionsManager } from './modules/features/smart-suggestions'
import { SubtasksManager } from './modules/features/subtasks'
import { TaskModalManager } from './modules/features/task-modal'
import { TaskOperations } from './modules/features/task-operations'
import { TemplatesManager } from './modules/features/templates'
import { TimeTrackingManager } from './modules/features/time-tracking'
import { UndoRedoManager } from './modules/features/undo-redo'
import { WeeklyReviewManager } from './modules/features/weekly-review'
import { ContextMenuManager } from './modules/ui/context-menu'
import { DarkModeManager } from './modules/ui/dark-mode'
import { MobileNavigationManager } from './modules/ui/mobile-navigation'
...
class GTDApp {
    ...
    helpQueue: Array<() => void>
    lastAnalyticsTime: number
    analyticsTimeout: number | null
    isShowingAnalytics: boolean
    analyticsQueue: Array<() => void>
    lastExportTime: number
    exportTimeout: number | null
    isExporting: boolean
    exportQueue: Array<() => void>
    lastImportTime: number
    importTimeout: number | null
    isImporting: boolean
    importQueue: Array<() => void>
    lastArchiveTime: number
    archiveTimeout: number | null
    isArchiving: boolean
    archiveQueue: Array<() => void>
    lastTemplateTime: number
    templateTimeout: number | null
    isUsingTemplate: boolean
    templateQueue: Array<() => void>
    lastCalendarTime: number
    calendarTimeout: number | null
    isShowingCalendar: boolean
    calendarQueue: Array<() => void>
    lastReviewTime: number
    reviewTimeout: number | null
    isInReview: boolean
    reviewQueue: Array<() => void>
    lastDashboardTime: number
    dashboardTimeout: number | null
    isShowingDashboard: boolean
    dashboardQueue: Array<() => void>
    lastSearchTime: number
    searchTimeout: number | null
    isSearching: boolean
    searchQueue: Array<() => void>
    lastContextMenuTime: number
    contextMenuTimeout: number | null
    isShowingContextMenu: boolean
    contextMenuQueue: Array<() => void>
    lastDependencyTime: number
    dependencyTimeout: number | null
    isShowingDependencies: boolean
    dependencyQueue: Array<() => void>
    lastQuickCaptureTime: number
    quickCaptureTimeout: number | null
    isInQuickCapture: boolean
    quickCaptureQueue: Array<() => void>
    lastNavigationTime: number
    navigationTimeout: number | null
    isNavigating: boolean
    navigationQueue: Array<() => void>
    lastProjectModalTime: number
    projectModalTimeout: number | null
    isShowingProjectModal: boolean
    projectModalQueue: Array<() => void>
    lastTaskModalTime: number
    taskModalTimeout: number | null
    isShowingTaskModal: boolean
    taskModalQueue: Array<() => void>
    lastBulkOperationTime: number
    bulkOperationTimeout: number | null
    isInBulkOperation: boolean
    bulkOperationQueue: Array<() => void>
    lastKeyboardNavTime: number
    keyboardNavTimeout: number | null
    isInKeyboardNav: boolean
    keyboardNavQueue: Array<() => void>
    lastDarkModeTime: number
    darkModeTimeout: number | null
    isInDarkMode: boolean
    darkModeQueue: Array<() => void>
    lastTimeTrackingTime: number
    timeTrackingTimeout: number | null
    isTimeTracking: boolean
    timeTrackingQueue: Array<() => void>
    lastFocusPomodoroTime: number
    focusPomodoroTimeout: number | null
    isInFocusPomodoro: boolean
    focusPomodoroQueue: Array<() => void>
    lastGlobalQuickCaptureTime: number
    globalQuickCaptureTimeout: number | null
    isInGlobalQuickCapture: boolean
    globalQuickCaptureQueue: Array<() => void>
    lastPriorityScoringTime: number
    priorityScoringTimeout: number | null
    isCalculatingPriorityScore: boolean
    priorityScoringQueue: Array<() => void>
    lastProductivityHeatmapTime: number
    productivityHeatmapTimeout: number | null
    isShowingProductivityHeatmap: boolean
    productivityHeatmapQueue: Array<() => void>

    lastUndoRedoTime: number
    undoRedoTimeout: number | null
    isInUndoRedo: boolean
    undoRedoQueue: Array<() => void>
    lastMobileNavigationTime: number
    mobileNavigationTimeout: number | null
    isInMobileNavigation: boolean
    mobileNavigationQueue: Array<() => void>

    lastModalHelpersTime: number
    modalHelpersTimeout: number | null
    isUsingModalHelpers: boolean
    modalHelpersQueue: Array<() => void>

    // Missing properties from constructor
    usageStats: unknown
    defaultContexts: string[]
    activeTimers: Map<string, unknown>
    calendarView: string
    calendarDate: Date
    ...
    async init () {
        ...
        try {
            ...
            // PWA manifest is included in build, service worker removed for single-file build
            this._initStep = 'PWA Setup'
    ...
    setupCustomTagHandler () {
        ...
        const saveCustomTag = (context) => {
            ...
            const isDuplicate = Array.from(allContexts).some(
    ...
    renderDefaultContextButtons () {
        ...
        if (quickContextsContainer) {
            ...
            this.defaultContexts.forEach((context) => {
                ...
                const count = contextTaskCounts[context] || 0
                btn.textContent = count > 0 ? `${context} (${count})` : context
        ...
        if (quickContextsContainer) {
            ...
            customContexts.forEach((context) => {
                ...
                btn.addEventListener('click', (e) => {
                    ...
                    if ((e.target as HTMLElement).classList.contains('custom-context-delete')) {
                        return
                    }
    ...
    createTaskElement (task, _index) {
        ...
        if (task.status === 'waiting' && task.waitingForDescription) {
            parts.push(
                `<i class="fas fa-hourglass-half"></i> Waiting: ${escapeHtml(task.waitingForDescription)}`
            )
        }
    ...
    renderProjectsDropdown () {
        ...
        sortedProjects.forEach((project) => {
            ...
            item.addEventListener('click', (e) => {
                ...
                if (
                    (e.target as HTMLElement).closest('.project-dropdown-item').dataset
                        .preventClick === 'true'
                ) {
                    ;(e.target as HTMLElement).closest(
                        '.project-dropdown-item'
                    ).dataset.preventClick = 'false'
                    return
                }
                this.viewProjectTasks(project.id)
            })
