...
import { StorageOperations } from './modules/core/storage-ops'

// View modules
import { TaskRenderer } from './modules/views/task-renderer'
import { ProjectRenderer } from './modules/views/project-renderer'
import { ViewManager } from './modules/views/view-manager'
...
class GTDApp {
    ...
    initializeCustomContexts () {
        ...
        if (customContexts) {
            try {
                ...
                this.state.defaultContexts = [
                    ...new Set([...this.state.defaultContexts, ...contexts])
                ]
            } catch (e) {
    ...
    setupEventListeners () {
        ...
        this.setupModalCloseButtons('project-modal', ['close-project-modal'], () =>
    ...
    renderProjectsDropdown () {
        ...
        dropdowns.forEach((dropdown) => {
            ...
            if (
                currentValue[dropdown.id] &&
                this.state.projects.find((p) => p.id === currentValue[dropdown.id])
            ) {
