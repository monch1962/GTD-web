...
import { getAllContexts, getContextTaskCounts, getContextIds } from '../js/config/defaultContexts.js';
...
describe('Context Utilities', () => {
    ...
    describe('getContextTaskCounts function', () => {
        ...
        test('should count tasks with multiple contexts in each context', () => {
            const tasks = [
                { id: '1', contexts: ['@home', '@work', '@computer'] }
            ];
    ...
    describe('DRY Compliance', () => {
        ...
        test('should not have inline context collection logic in app.js', () => {
            ...
            const methodMatch = appContent.match(/updateSidebarContextFilters\(\)\s*\{([\s\S]*?)\n    \}/);
            expect(methodMatch).toBeTruthy();

            const methodBody = methodMatch[1];

            // Should NOT have manual custom context collection
            const hasManualCustomCollection = methodBody.includes('customContexts') &&
            ...
            const hasManualTaskCounting = methodBody.includes('contextTaskCounts = {}') &&
                                          methodBody.includes('this.tasks.forEach') &&
        ...
        test('no other JS files should duplicate context combining logic', () => {
            ...
            jsFiles.forEach(filePath => {
                ...
                const manualCollectionPattern = /this\.tasks\.forEach\([\s\S]*?contexts\.forEach[\s\S]*?\!\s*defaultContexts\.includes/s;
                ...
                const manualCountingPattern = /contextTaskCounts\s*=\s*\{\}[\s\S]*?this\.tasks\.forEach[\s\S]*?contextTaskCounts\[/;
                if (manualCountingPattern.test(content) && !content.includes('getContextTaskCounts')) {
                    violations.push({
                        file: relativePath,
