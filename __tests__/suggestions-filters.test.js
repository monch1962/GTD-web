...
describe('Suggestions Modal Filters', () => {
    ...
    describe('getSmartSuggestions uses filters correctly', () => {
        ...
        test('should filter tasks by context with score boost', () => {
            ...
            const contextMatch = appContent.match(
                /Context match[\s\S]*?if\s*\(context\s*&&[\s\S]*?includes\(context\)/
            )
        ...
        test('should filter tasks by energy level with score boost', () => {
            ...
            const energyMatch = appContent.match(
                /Energy level match[\s\S]*?if\s*\(energyLevel[\s\S]*?Matches your energy level/
            )
        ...
        test('should filter tasks by available time', () => {
            ...
            const timeMatch = appContent.match(
                /Time available match[\s\S]*?Fits your available time/
            )
