...
exports.ProjectRenderer = void 0
...
class ProjectRenderer {
    ...
    renderProjects(container) {
        ...
        filteredProjects.sort((a, b) => {
            const statusOrder = { active: 0, someday: 1, completed: 2 }
    ...
    createProjectCard(project) {
        ...
        const taskCount = this.state.tasks
            ? this.state.tasks.filter((t) => t.projectId === project.id && !t.completed).length
            : 0
        // Build context tags
        const contextTags =
            project.contexts && project.contexts.length > 0
                ? project.contexts
                      .map(
                          (ctx) =>
                              `<span class="project-context">${(0, dom_utils_1.escapeHtml)(ctx)}</span>`
                      )
                      .join('')
                : ''
        card.innerHTML = `
            <div class="project-card-header">
                <div class="project-title">${(0, dom_utils_1.escapeHtml)(project.title)}</div>
                <div class="project-status-badge">${project.status}</div>
            </div>
            <div class="project-card-body">
                ${project.description ? `<div class="project-description">${(0, dom_utils_1.escapeHtml)(project.description)}</div>` : ''}
            ...
            <div class="project-card-actions">
                ...
                <button class="project-action-btn ${project.status === 'archived' ? 'unarchive' : 'archive'}" title="${project.status === 'archived' ? 'Unarchive' : 'Archive'} project">
     ...
     * @param project - Project object
     */
    _attachProjectCardListeners(card, project) {
...
//# sourceMappingURL=project-renderer.js.map
