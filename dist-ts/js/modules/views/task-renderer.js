...
const virtual_scroll_1 = require('../ui/virtual-scroll')
...
class TaskRenderer {
    constructor(state, app) {
        ...
        this.logger = (0, logger_1.createLogger)('TaskRenderer')
    }
    /**
    ...
    renderTasks(container, filterFn = null) {
        ...
        if (filteredTasks.length >= constants_1.VirtualScrollConfig.ACTIVATION_THRESHOLD) {
    ...
    _renderWithVirtualScroll(container, tasks) {
        ...
        this.logger.debug(
            `Virtual scrolling ACTIVATED: ${tasks.length} tasks (threshold: ${constants_1.VirtualScrollConfig.ACTIVATION_THRESHOLD})`
        )
    ...
    _renderRegular(container, tasks) {
        ...
        if (this.virtualScroll) {
            this.logger.debug(
                `Virtual scrolling DEACTIVATED: ${tasks.length} tasks (< ${constants_1.VirtualScrollConfig.ACTIVATION_THRESHOLD} threshold)`
            )
    ...
    _initializeVirtualScroll(container) {
        ...
        this.virtualScroll = new virtual_scroll_1.VirtualScrollManager(container, {
            itemHeight: constants_1.VirtualScrollConfig.ITEM_HEIGHT,
            bufferSize: constants_1.VirtualScrollConfig.BUFFER_ITEMS,
            scrollDebounce: constants_1.VirtualScrollConfig.DEBOUNCE_DELAY
        })
    }
    /**
     * Create task element
    ...
    _buildTaskHTML(task) {
        ...
        const contextTags =
            task.contexts && task.contexts.length > 0
                ? task.contexts
                      .map(
                          (ctx) =>
                              `<span class="task-context">${(0, dom_utils_1.escapeHtml)(ctx)}</span>`
                      )
                      .join('')
                : ''
        ...
        if (task.projectId) {
            ...
            if (project) {
                projectIndicator = `<span class="task-project">${(0, dom_utils_1.escapeHtml)(project.title)}</span>`
            }
        }
                    ...
                    task.subtasks && task.subtasks.length > 0
                        ...
                        ${task.subtasks
                            ...
                            <div class="task-subtask ${sub.completed ? 'completed' : ''}">
                                ...
                                <span>${(0, dom_utils_1.escapeHtml)(sub.title)}</span>
                            </div>
                        `
                            )
                            .join('')}
    ...
    _attachTaskListeners(element, task) {
        ...
        if (moreBtn) {
            moreBtn.addEventListener('click', (e) => {
                e.stopPropagation()
                this._showTaskContextMenu(e, task, element)
            })
        }
        // Timer button
        ...
        element.addEventListener('click', (e) => {
            ...
            if (
                e.target.closest('.task-complete-checkbox') ||
                e.target.closest('.task-star') ||
                e.target.closest('.task-action-btn') ||
                e.target.closest('.task-timer')
            ) {
                return
            }
     ...
     * @param element - Task element
     */
    _showTaskContextMenu(event, task, element) {
        // Save state for undo
     ...
     */
    _toggleSubtask(task, subtaskIndex) {
