...
const task_renderer_1 = require('./task-renderer')
const project_renderer_1 = require('./project-renderer')
class ViewManager {
    constructor(state, app) {
        this.state = state
        this.app = app
        this.taskRenderer = new task_renderer_1.TaskRenderer(state, app)
        this.projectRenderer = new project_renderer_1.ProjectRenderer(state, app)
    }
    /**
    ...
    viewProjectTasks(projectId) {
        ...
        if (project && viewTitle) {
            ...
            const ganttButton = document.getElementById('show-gantt-chart')
            if (ganttButton) {
    ...
    renderReference() {
        ...
        references.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
        // Render references
    ...
    updateCounts() {
        const counts = {
            ...
            next: this.state.tasks.filter(
                (t) => t.status === 'next' && !t.completed && t.areDependenciesMet(this.state.tasks)
            ).length,
            waiting: this.state.tasks.filter((t) => t.status === 'waiting' && !t.completed).length,
            someday: this.state.tasks.filter((t) => t.status === 'someday' && !t.completed).length,
        ...
        this._updateCount(
            'reference-count',
            this.state.tasks.filter((t) => t.type === 'reference').length
        )
    ...
    updateContextFilter() {
        ...
        const currentValue = contextFilter.getAttribute('value') || ''
        ...
        contextFilter.setAttribute('value', currentValue)
    }
    /**
    ...
    _updateViewTitle(view) {
        const baseTitle = constants_1.ViewLabels[view] || view
        let title = baseTitle
    ...
    _toggleContainers(view) {
        ...
        if (view === 'projects') {
            ...
            if (referenceContainer) referenceContainer.style.display = 'block'
        } else {
        ...
        div.innerHTML = `
            <div class="reference-content">
                <div class="reference-title">
                    <a href="${(0, dom_utils_1.escapeHtml)(ref.url || '')}" target="_blank" rel="noopener noreferrer">
                        ${(0, dom_utils_1.escapeHtml)(ref.title)}
                    </a>
                </div>
                ${ref.description ? `<div class="reference-description">${(0, dom_utils_1.escapeHtml)(ref.description)}</div>` : ''}
                ${
                    ref.contexts && ref.contexts.length > 0
                        ? `
                    <div class="reference-contexts">
                        ${ref.contexts.map((ctx) => `<span class="task-context">${(0, dom_utils_1.escapeHtml)(ctx)}</span>`).join('')}
                    </div>
                `
                        : ''
                }
