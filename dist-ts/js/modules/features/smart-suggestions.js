...
exports.SmartSuggestionsManager = void 0
...
class SmartSuggestionsManager {
    ...
    showSuggestions() {
        ...
        const contextOptions = sortedContexts
            .map(
                (context) =>
                    `<option value="${(0, dom_utils_1.escapeHtml)(context)}">${(0, dom_utils_1.escapeHtml)(context)}</option>`
            )
            .join('\n                                ')
        modal.innerHTML = `
                ...
                const contexts =
                    task.contexts && task.contexts.length > 0
                        ? task.contexts
                              .map(
                                  (c) =>
                                      `<span class="task-context">${(0, dom_utils_1.escapeHtml)(c)}</span>`
                              )
                              .join(' ')
                        : ''
...
exports.SmartSuggestionsManager = SmartSuggestionsManager
