...
exports.FocusPomodoroManager = void 0
const dom_utils_1 = require('../../dom-utils')
class FocusPomodoroManager {
    ...
    renderFocusTask(task) {
        ...
        if (!container) return
        container.innerHTML = `
            <h1 style="font-size: 2.5rem; margin-bottom: var(--spacing-lg); text-align: center;">${(0, dom_utils_1.escapeHtml)(task.title)}</h1>
            <div style="background: var(--bg-secondary); padding: var(--spacing-lg); border-radius: var(--radius-md); max-width: 600px; width: 100%; margin-bottom: var(--spacing-lg);">
                ${task.description ? `<p style="margin-bottom: var(--spacing-md);">${(0, dom_utils_1.escapeHtml)(task.description)}</p>` : ''}
                <div style="display: flex; gap: var(--spacing-sm); flex-wrap: wrap; margin-bottom: var(--spacing-sm);">
                    ${task.contexts ? task.contexts.map((c) => `<span style="background: var(--primary-color); color: white; padding: 4px 8px; border-radius: 12px;">${(0, dom_utils_1.escapeHtml)(c)}</span>`).join('') : ''}
                    ...
                    <div style="background: var(--bg-secondary); padding: var(--spacing-md); border-radius: var(--radius-md);">
                        ${task.subtasks
                            ...
                            <label style="display: flex; align-items: center; gap: var(--spacing-sm); padding: 8px 0; cursor: pointer;">
                                ...
                                <span style="${subtask.completed ? 'text-decoration: line-through; opacity: 0.6;' : ''}">${(0, dom_utils_1.escapeHtml)(subtask.title)}</span>
                            </label>
                        `
                            )
                            .join('')}
            ...
            ${
                ...
                <div style="max-width: 600px; width: 100%;">
                    ...
                    <div style="background: var(--bg-secondary); padding: var(--spacing-md); border-radius: var(--radius-md); white-space: pre-wrap;">
                        ${(0, dom_utils_1.escapeHtml)(task.notes)}
                    </div>
                </div>
            `
                    : ''
...
exports.FocusPomodoroManager = FocusPomodoroManager
//# sourceMappingURL=focus-pomodoro.js.map
