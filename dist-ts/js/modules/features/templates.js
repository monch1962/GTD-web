...
class TemplatesManager {
        ...
        if (modal) {
            ...
            if (templateId) {
                ...
                if (template) {
                    ...
                    const energyInput = document.getElementById('template-energy')
                    const timeInput = document.getElementById('template-time')
    ...
    async handleTemplateFormSubmit(e) {
        ...
        if (templateId) {
            ...
            // Create new template
            const newTemplate = new models_1.Template(templateData)
                ...
                html += `
                    <div class="template-card" style="background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: var(--spacing-md);">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: var(--spacing-sm);">
                            <h4 style="margin: 0; flex: 1;">${(0, dom_utils_1.escapeHtml)(template.title)}</h4>
                        ...
                        ${template.description ? `<p style="color: var(--text-secondary); margin: var(--spacing-xs) 0; font-size: 0.9rem;">${(0, dom_utils_1.escapeHtml)(template.description)}</p>` : ''}
                        <div style="display: flex; flex-wrap: wrap; gap: var(--spacing-xs); margin-bottom: var(--spacing-sm);">
                            ...
                            ${template.contexts.map((ctx) => `<span class="badge" style="background: var(--context-color);">${(0, dom_utils_1.escapeHtml)(ctx)}</span>`).join('')}
                        </div>
    ...
    renderTemplateContexts(selectedContexts = []) {
        ...
        container.innerHTML = allContexts
            .map((context) => {
                ...
                return `
                <button type="button" class="context-btn ${isSelected ? 'active' : ''}" data-context="${(0, dom_utils_1.escapeHtml)(context)}">
                    ${(0, dom_utils_1.escapeHtml)(context)}
                </button>
            `
            })
            .join('')
    ...
    renderTemplateSubtasks(subtasks = []) {
        ...
        container.innerHTML = subtasks
            ...
            <div class="subtask-item" style="display: flex; gap: var(--spacing-sm); margin-bottom: var(--spacing-xs); align-items: center;">
                <input type="text" class="form-control" value="${(0, dom_utils_1.escapeHtml)(subtask.title)}" data-index="${index}" placeholder="Subtask title">
