...
class StorageOperations {
    ...
    async loadData() {
        ...
        this.state.tasks = tasksData.map((data) => models_1.Task.fromJSON(data))
        // Load projects
        const projectsData = this.storage.getProjects()
        this.state.projects = projectsData.map((data) => models_1.Project.fromJSON(data))
        // Load templates
        const templatesData = this.storage.getTemplates()
        this.state.templates = templatesData.map((data) => models_1.Template.fromJSON(data))
    }
    /**
